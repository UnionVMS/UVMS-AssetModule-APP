...
import eu.europa.ec.fisheries.uvms.mobileterminal.constants.ParameterKey;
...
public class GetReceivedEventBean {
    ...
    public MobileTerminalType getMobileTerminal(EventMessage message) {
        ...
        AssetDataSourceQueue dataSource = AssetDataSourceQueue.INTERNAL;
        try {
        ...
        } catch (AssetException | AssetMessageException ex) {
            mobTerm = null;
        }
        if (mobTerm == null) {
            LOG.debug("Trying to retrieve MobileTerminal from datasource: {0} as second option", AssetDataSourceQueue.INTERNAL.name());
            try {
                request = JAXBMarshaller.unmarshallTextMessage(message.getJmsMessage(), GetMobileTerminalRequest.class);
                mobTerm = service.getMobileTerminalByIdFromInternalOrExternalSource(request.getId(), AssetDataSourceQueue.INTERNAL);
            } catch (AssetException | AssetMessageException ex) {
    ...
    private AssetDataSourceQueue decideDataflow() throws ConfigServiceException {
        ...
        if (national) {
            return AssetDataSourceQueue.INTEGRATION;
        }
